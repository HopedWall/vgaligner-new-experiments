...
DATASETS = [name for name in os.listdir("./datasets") if os.path.isdir(name) and not name.startswith(".")]
...
rule vg_map:
	input:
		...
		reads="datasets/{dataset}/reads/{path}_0001.fastq"
	output:
		temporary("datasets/{dataset}/results/{path}_vgmap.gam")
	params:
...
rule vg_map_convert_graphchainer:
	input: 
		gam="datasets/{dataset}/results/{path}_graphchainer.gam",
		graph="datasets/{dataset}/sorted_graph.gfa"
	output:
		"datasets/{dataset}/results/{path}_graphchainer.gaf"
	shell:
